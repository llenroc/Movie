...
namespace Infrastructure.Configuration
{
    ...
    public class SettingManager : ISettingManager, ISingletonDependency
    {
        ...
        {
            return GetSettingValueInternalAsync(name, Session.TenantId, Session.UserId);
        }
        ...
        public async Task<IReadOnlyList<ISettingValue>> GetAllSettingValuesAsync(SettingScopes scopes)
        {
            ...
            if (scopes.HasFlag(SettingScopes.Application))
            {
                foreach (var settingValue in await GetAllSettingValuesForApplicationAsync())
                {
                    ...
                    if (!setting.IsInherited &&((setting.Scopes.HasFlag(SettingScopes.Tenant) && Session.TenantId.HasValue) || (setting.Scopes.HasFlag(SettingScopes.User) && Session.UserId.HasValue)))
                    {
                        continue;
                    }
            ...
            if (scopes.HasFlag(SettingScopes.Tenant) && Session.TenantId.HasValue)
            {
                foreach (var settingValue in await GetAllSettingValuesForTenantAsync(Session.TenantId.Value))
                {
                    ...
                    if (!setting.IsInherited &&(setting.Scopes.HasFlag(SettingScopes.User) && Session.UserId.HasValue))
                    {
                        continue;
                    }
                    settingValues[settingValue.Name] = new SettingValueObject(settingValue.Name, settingValue.Value);
        ...
        {
            return (await GetApplicationSettingsAsync()).Values.Select(setting => new SettingValueObject(setting.Name, setting.Value)).ToImmutableList();
        }

        /// <inheritdoc/>
        public async Task<IReadOnlyList<ISettingValue>> GetAllSettingValuesForTenantAsync(int tenantId)
        {
            return (await GetReadOnlyTenantSettings(tenantId)).Values.Select(setting => new SettingValueObject(setting.Name, setting.Value)).ToImmutableList();
        }

        /// <inheritdoc/>
        public Task<IReadOnlyList<ISettingValue>> GetAllSettingValuesForUserAsync(long userId)
        {
            return GetAllSettingValuesForUserAsync(new UserIdentifier(Session.TenantId, userId));
        }

        public async Task<IReadOnlyList<ISettingValue>> GetAllSettingValuesForUserAsync(UserIdentifier user)
        {
            return (await GetReadOnlyUserSettings(user)).Values.Select(setting => new SettingValueObject(setting.Name, setting.Value)).ToImmutableList();
        }
        ...
        {
            return ChangeSettingForUserAsync(new UserIdentifier(Session.TenantId, userId), name, value);
        }
