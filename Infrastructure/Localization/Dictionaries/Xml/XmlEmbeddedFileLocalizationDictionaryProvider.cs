...
namespace Infrastructure.Localization.Dictionaries.Xml
{
    /// <summary>
            ...
            foreach (var resourceName in resourceNames)
            {
                if (resourceName.StartsWith(_rootNamespace))
                {
                    using (var stream = _assembly.GetManifestResourceStream(resourceName))
                    {
                        ...
                        {
                            throw new InitializationException(sourceName + " source contains more than one dictionary for the culture: " + dictionary.CultureInfo.Name);
                        }
                        ...
                        if (resourceName.EndsWith(sourceName + ".xml"))
                        {
                            ...
                            {
                                throw new InitializationException("Only one default localization dictionary can be for source: " + sourceName);
                            }
