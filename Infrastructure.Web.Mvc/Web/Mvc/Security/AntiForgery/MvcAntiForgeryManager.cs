...
{
    public class MvcAntiForgeryManager : InfrastructureAntiForgeryManager
    {
        private static readonly Lazy<object> AntiForgeryWorkerObject = new Lazy<object>(() =>
        {
            ...
            {
                throw new Exception("Can not get _worker field of System.Web.Helpers.AntiForgery class. It's internal implementation might be changed. Please create an issue on Github repository to solve this.");
            }
        ...
        {
            return AntiForgeryWorkerObject.Value.GetType().GetMethod("GetFormInputElement", BindingFlags.Public | BindingFlags.Instance);
        });
