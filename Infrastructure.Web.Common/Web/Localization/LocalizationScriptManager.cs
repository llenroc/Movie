...
namespace Infrastructure.Web.Localization
{
    internal class LocalizationScriptManager : ILocalizationScriptManager, ISingletonDependency
    {
        ...
        public string GetScript(CultureInfo cultureInfo)
        {
            ...
            //return _cacheManager.GetCache(CacheNames.LocalizationScripts).Get(cultureInfo.Name, () => BuildAll(cultureInfo));
        }
        ...
        private string BuildAll(CultureInfo cultureInfo)
        {
            ...
            script.AppendLine();
            script.AppendLine("    infrastructure.localization = infrastructure.localization || {};");
            script.AppendLine();
            script.AppendLine("    infrastructure.localization.currentCulture = {");
            script.AppendLine("        name: '" + cultureInfo.Name + "',");
            script.AppendLine("        displayName: '" + cultureInfo.DisplayName + "'");
            script.AppendLine("    };");
            script.AppendLine();
            script.Append("    infrastructure.localization.languages = [");
            ...
            if (languages.Count > 0)
            {
                ...
                script.AppendLine("    infrastructure.localization.currentLanguage = {");
            ...
            script.AppendLine();
            script.AppendLine("    infrastructure.localization.sources = [");
            ...
            script.AppendLine();
            script.AppendLine("    infrastructure.localization.values = infrastructure.localization.values || {};");
            script.AppendLine();

            foreach (var source in sources)
            {
                script.Append("    infrastructure.localization.values['" + source.Name + "'] = ");
