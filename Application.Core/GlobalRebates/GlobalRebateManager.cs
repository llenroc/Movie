    ...
    public class GlobalRebateManager : ApplicationDomainServiceBase
    {
        ...
        public async Task TryAndCreateOrderGlobalRebateAsync(Order order)
        {
            ...
            foreach(GlobalRebate globalRebate in globalRebates)
            {
                ...
                await WalletManager.IncomeOfOrderRebate(globalRebate.User.ToUserIdentifier(), money, L("GlobalRebate"), order);
                await WalletManager.WithdrawAllBalanceAsync(globalRebate.User.ToUserIdentifier());
            }
        }
    }
}
