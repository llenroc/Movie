...
using Infrastructure.WebApi.Controllers.Dynamic.Scripting.Angular;

namespace Infrastructure.WebApi.Controllers.Dynamic.Scripting.TypeScript
{
    internal class TypeScriptServiceProxyGenerator : ITransientDependency
    {
        ...
        public string Generate(DynamicApiControllerInfo controllerInfo, string servicePrefix)
        {
            ...
            foreach (var methodInfo in _controllerInfo.Actions.Values)
            {
                ...
                if (returnType == "void")
                {
                    script.AppendLine(string.Format("           public {0} = function ({1}): infrastructure.IPromise ",
                    ...
                    script.AppendLine("                        infrastructure: true,");
                    script.AppendLine("                        url: infrastructure.appPath + '" + ActionScriptingHelper.GenerateUrlWithParameters(_controllerInfo, methodInfo) + "',");
                ...
                {
                    script.AppendLine(string.Format("           public {0} = function ({1}): infrastructure.IGenericPromise<{2}> ", methodInfo.ActionName.ToCamelCase(), 
            ...
            script.AppendLine("angular.module('infrastructure').service('infrastructure.services." + _controllerInfo.ServiceName.Replace("/", ".") + "', infrastructure.services." + _controllerInfo.ServiceName.Replace("/", ".")+");");
