...
namespace Infrastructure.WebApi.Controllers.Dynamic.Selectors
{
    ...
    /// </summary>
    public class InfrastructureApiControllerActionSelector : ApiControllerActionSelector
    {
        ...
        public override HttpActionDescriptor SelectAction(HttpControllerContext controllerContext)
        {
            ...
            if (!controllerContext.ControllerDescriptor.Properties.TryGetValue("__InfrastructureDynamicApiControllerInfo", out controllerInfoObj))
            {
                return GetDefaultActionDescriptor(controllerContext);
            }

            //Get controller information which is selected by InfrastructureHttpControllerSelector.
            var controllerInfo = controllerInfoObj as DynamicApiControllerInfo;

            if (controllerInfo == null)
            {
                throw new InfrastructureException("__InfrastructureDynamicApiControllerInfo in ControllerDescriptor.Properties is not a " + typeof(DynamicApiControllerInfo).FullName + " class.");
            }

            //No action name case
            var hasActionName = (bool)controllerContext.ControllerDescriptor.Properties["__InfrastructureDynamicApiHasActionName"];
        ...
        private HttpActionDescriptor GetActionDescriptorByActionName(HttpControllerContext controllerContext, DynamicApiControllerInfo controllerInfo, string actionName)
        {
            ...
            {
                throw new InfrastructureException("There is no action " + actionName + " defined for api controller " + controllerInfo.ServiceName);
            }
