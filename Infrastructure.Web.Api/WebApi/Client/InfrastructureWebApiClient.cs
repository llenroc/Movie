...
{
    public class InfrastructureWebApiClient : ITransientDependency, IInfrastructureWebApiClient
    {
        ...
        static InfrastructureWebApiClient()
        {
            DefaultTimeout = TimeSpan.FromSeconds(90);
        }

        public InfrastructureWebApiClient()
        {
        ...
        public virtual async Task<TResult> PostAsync<TResult>(string url, int? timeout = null) where TResult : class
        {
            return await PostAsync<TResult>(url, null, timeout);
        }

        public virtual async Task<TResult> PostAsync<TResult>(string url, object input, int? timeout = null) where TResult : class
        {
                ...
                using (var client = new HttpClient(handler))
                {
                    ...
                    using (var requestContent = new StringContent(Object2JsonString(input), Encoding.UTF8, "application/json"))
                    {
                        ...
                        using (var response = await client.PostAsync(url, requestContent))
                        {
                            ...
                            {
                                throw new InfrastructureException("Could not made request to " + url + "! StatusCode: " + response.StatusCode + ", ReasonPhrase: " + response.ReasonPhrase);
                            }
                            ...
                            {
                                throw new InfrastructureRemoteCallException(ajaxResponse.Error);
                            }
