...
{
    /// <summary>
    /// Extends <see cref="CommonFrameUserManager{TUser,TKey}"/> of ASP.NET Identity Framework.
    /// </summary>
    public abstract class CommonFrameUserManager<TRole, TUser>: UserManager<TUser, long>, 
        IDomainService where TRole : CommonFrameRole<TUser>, new() where TUser : CommonFrameUser<TUser>
    {
        protected IUserPermissionStore<TUser> UserPermissionStore
        {
            get
            {
                ...
                {
                    throw new InfrastructureException("Store is not IUserPermissionStore");
                }
        ...
        protected CommonFrameRoleManager<TRole, TUser> RoleManager { get; }

        public CommonFrameUserStore<TRole, TUser> Store { get; }

        protected readonly IPermissionManager _permissionManager;
        protected readonly IUnitOfWorkManager _unitOfWorkManager;
        protected readonly ICacheManager _cacheManager;
        protected readonly IRepository<OrganizationUnit, long> _organizationUnitRepository;
        protected readonly IRepository<UserOrganizationUnit, long> _userOrganizationUnitRepository;
        protected readonly IOrganizationUnitSettings _organizationUnitSettings;
        protected readonly ISettingManager _settingManager;

        protected CommonFrameUserManager(
            CommonFrameUserStore<TRole, TUser> userStore,
        ...
        public virtual async Task<bool> IsGrantedAsync(long userId, Permission permission)
        {
            ...
            if (!permission.MultiTenancySides.HasFlag(Session.MultiTenancySide))
            {
                return false;
            }

            //Check for depended features
            if (permission.FeatureDependency != null && Session.MultiTenancySide == MultiTenancySides.Tenant)
            {
        ...
        public virtual async Task<TUser> FindByNameOrEmailAsyncOfAll(string userNameOrEmailAddress)
        {
        ...
        public async override Task<ClaimsIdentity> CreateIdentityAsync(TUser user, string authenticationType)
        {
            ...
            {
                identity.AddClaim(new Claim(InfrastructureClaimTypes.TenantId, user.TenantId.Value.ToString(CultureInfo.InvariantCulture)));
            }
            return identity;
        }
        ...
        {
            return await IsInOrganizationUnitAsync(await GetUserByIdAsync(userId),await _organizationUnitRepository.GetAsync(ouId));
        }

        public virtual async Task<bool> IsInOrganizationUnitAsync(TUser user, OrganizationUnit ou)
        {
            return await _userOrganizationUnitRepository.CountAsync(uou =>uou.UserId == user.Id && uou.OrganizationUnitId == ou.Id) > 0;
        }

        public virtual async Task AddToOrganizationUnitAsync(long userId, long ouId)
        {
            await AddToOrganizationUnitAsync( await GetUserByIdAsync(userId),await _organizationUnitRepository.GetAsync(ouId)
 );
        }
        ...
        {
            await RemoveFromOrganizationUnitAsync(await GetUserByIdAsync(userId), await _organizationUnitRepository.GetAsync(ouId));
        }
        ...
        {
            await SetOrganizationUnitsAsync( await GetUserByIdAsync(userId),  organizationUnitIds);
        }
        ...
        private async Task CheckMaxUserOrganizationUnitMembershipCountAsync(int? tenantId, int requestedCount)
        {
            ...
            {
                throw new Exception(string.Format("Can not set more than {0} organization unit for a user!", maxCount));
            }
        }
        ...
        public virtual async Task SetOrganizationUnitsAsync(TUser user, params long[] organizationUnitIds)
        {
            ...
            foreach (var organizationUnitId in organizationUnitIds)
            {
                ...
                {
                    await AddToOrganizationUnitAsync(user, await _organizationUnitRepository.GetAsync(organizationUnitId) );
                }
            }
        }
        ...
        public virtual void RegisterTwoFactorProviders(int? tenantId)
        {
            ...
            if (!IsTrue(CommonFrameSettingNames.UserManagement.TwoFactorLogin.IsEnabled, tenantId))
            {
                return;
            }

            if (EmailService != null && IsTrue(CommonFrameSettingNames.UserManagement.TwoFactorLogin.IsEmailProviderEnabled, tenantId))
            {
            ...
            if (SmsService != null &&IsTrue(CommonFrameSettingNames.UserManagement.TwoFactorLogin.IsSmsProviderEnabled, tenantId))
            {
                RegisterTwoFactorProvider(
                    L("Sms"),
        ...
        {
            UserLockoutEnabledByDefault = IsTrue(CommonFrameSettingNames.UserManagement.UserLockOut.IsEnabled, tenantId);
            DefaultAccountLockoutTimeSpan = TimeSpan.FromSeconds(GetSettingValue<int>(CommonFrameSettingNames.UserManagement.UserLockOut.DefaultAccountLockoutSeconds, tenantId));
            MaxFailedAccessAttemptsBeforeLockout = GetSettingValue<int>(CommonFrameSettingNames.UserManagement.UserLockOut.MaxFailedAccessAttemptsBeforeLockout, tenantId);
        }
        ...
        {
            return tenantId == null ? _settingManager.GetSettingValueForApplication<T>(settingName) : _settingManager.GetSettingValueForTenant<T>(settingName, tenantId.Value);
        }
