...
{
    /// <summary>
    /// Extends <see cref="CommonFrameRoleManager{TRole,TKey}"/> of ASP.NET Identity Framework.
    /// Applications should derive this class with appropriate generic arguments.
    /// </summary>
    public abstract class CommonFrameRoleManager<TRole, TUser>: RoleManager<TRole, int>,IDomainService where TRole : CommonFrameRole<TUser>, new() where TUser : CommonFrameUser<TUser>
    {
        ...
        protected CommonFrameRoleStore<TRole, TUser> Store { get; private set; }
        ...
        /// Constructor.
        /// </summary>
        protected CommonFrameRoleManager(
            CommonFrameRoleStore<TRole, TUser> store,
        ...
        public virtual async Task<TRole> GetRoleByIdAsync(int roleId)
        {
            ...
            if (role == null)
            {
                throw new InfrastructureException("There is no role with id: " + roleId);
            }
            return role;
        }
        ...
        public virtual async Task<TRole> GetRoleByNameAsync(string roleName)
        {
            ...
            if (role == null)
            {
                throw new InfrastructureException("There is no role with name: " + roleName);
            }
            return role;
        }
